...
@interface GotyeChatViewController () <GotyeOCDelegate, UITableViewDataSource, UITableViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
...
- (void)viewDidLoad
{
    ...
    GotyeOCUser* user = nil;
    GotyeOCGroup* group = nil;
    ...
    switch (chatTarget.type) {
        case GotyeChatTargetTypeUser:
        {
            ...
            user = [GotyeOCAPI getUserDetail:chatTarget forceRequest:NO];
        ...
        case GotyeChatTargetTypeGroup:
        {
            ...
            group = [GotyeOCAPI getGroupDetail:chatTarget forceRequest:NO];
...
#pragma mark - 设置红包功能
    ...
    // 设置的是接收者的 id ，
    RedpacketUserInfo *redpacketUser = [[RedpacketUserInfo alloc] init];
    
    if (GotyeChatTargetTypeUser == chatTarget.type) {
        redpacketUser.userNickname = user.name;
        redpacketUser.userId = user.name;
    }
    else if(GotyeChatTargetTypeGroup == chatTarget.type) {
        redpacketUser.isGroup = YES;
        redpacketUser.userId = [NSString stringWithFormat:@"%lld", group.id];
    }
    
    self.redpacketControl.converstationInfo = redpacketUser;
    
    __weak GotyeChatViewController *SELF = self;
    // 设置红包 SDK 功能回调
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if(self.isMovingToParentViewController)
    {
        ...
        chatView.frame = CGRectMake(0, self.view.frame.size.height - 50, self.view.frame.size.width, 150);
        self.tableView.frame = CGRectMake(0, 0, self.view.frame.size.width, chatView.frame.origin.y);
...
- (IBAction)redpacketClick:(id)sender
{
    if (GotyeChatTargetTypeUser == chatTarget.type) {
        ...
        [self.redpacketControl presentRedPacketMoreViewControllerWithCount:0];
    }
}
...
- (void)onStopTalk:(GotyeStatusCode)code realtime:(bool)realtime message:(GotyeOCMessage*)message cancelSending:(bool *)cancelSending
{
    ...
    *cancelSending = YES;
    if(message.text != nil && message.text.length > 0){
        [message putExtraData: [message.text UTF8String] len: strlen([message.text UTF8String])];
    }
